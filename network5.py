...
class Network:
    ...
    def backwardPass(self,v):
        ...
        v[0] = self.beta*v[0] + (1-self.beta)*np.dot(temp2,self.layer_stack[0].dw.T)
        v[1] = self.beta*v[1] + (1-self.beta)*temp2
        v[2] = self.beta*v[2] + (1-self.beta)*np.dot(self.layer_stack[2].dw,temp1.T).T      
        v[3] = self.beta*v[3] + (1-self.beta)*temp1
        

        self.layer_stack[2].w = self.layer_stack[2].w - self.learning_rate*v[2]
        self.layer_stack[2].b = self.layer_stack[2].b - self.learning_rate*v[3]
        self.layer_stack[0].w = self.layer_stack[0].w - self.learning_rate*v[0]
        self.layer_stack[0].b = self.layer_stack[0].b - self.learning_rate*v[1]
    ...
    def train(self,data_x,data_y):
        count = 0
        v = []
        v.append(np.zeros(self.layer_stack[0].w.shape,dtype = np.float64))
        v.append(np.zeros(self.layer_stack[0].b.shape,dtype = np.float64))
        v.append(np.zeros(self.layer_stack[2].w.shape,dtype = np.float64))
        v.append(np.zeros(self.layer_stack[2].b.shape,dtype = np.float64))
