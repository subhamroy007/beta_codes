...
class Network:
    def __init__(self,input_size,layer_size,output_size,learning_rate,epochs):
    ...
    def backwardPass(self):
        #derive average loss over the batch
        temp3 = np.array(self.loss_list,dtype = np.float64)
        self.layer_stack[4].l = temp3.sum(axis = 0)
        self.layer_stack[4].l = self.layer_stack[4].l/temp3.shape[0]
    ...
    def train(self,data_x,data_y):
        start = 0 
        end = round(data_x.shape[0]/self.epochs)
        self.loss_list = []
        for i in range(self.epochs):
            print("running batch no: ",i)
            for x,y in zip(data_x[start:end,:,:],data_y[start:end,:,:]):
...
my_model = Network(12,24,2,.12,100)
my_model.train(x,y)
